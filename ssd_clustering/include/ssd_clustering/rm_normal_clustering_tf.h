...
void detection(Cluster cluster, CloudA cloud, amsl_recog_msgs::ObjectInfoArray& cluster_array)
{
    if( // 0.0<cluster.z &&
        0.2<cluster.width && cluster.width<1.2 &&
        0.5<cluster.height && cluster.height<2.0 &&
        0.05<cluster.depth && cluster.depth<1.2 ){

        printf("cluster centroid x:%.2f y:%.2f z:%.2f\n", cluster.x, cluster.y, cluster.z);
...
void clustering(CloudAPtr cloud_in, amsl_recog_msgs::ObjectInfoArray& cluster_array)
{
