...
import os
print (os.getcwd())
path = os.path.join(os.getcwd(), 'weights_SSD300.hdf5')
...
class SSDCamera(object):
    def __init__(self, class_names, model, input_shape):
        ...
        self.pub = rospy.Publisher('/realsense/ssd/BoxArray',ObjectInfoArray,queue_size=10)
        self.pub_ssd = rospy.Publisher('/realsense/ssd/Image', Image, queue_size=10)
    ...
    def run(self, start_frame = 0, conf_thresh = 0.6):
        ...
        # box_array = BoundingBoxArray() # changed by Y.S
        box_array = ObjectInfoArray()
        ...
        if len(results) > 0 and len(results[0]) > 0:
            ...
            for i in range(top_conf.shape[0]):
                ...
                # box = BoundingBox() # changed by Y.S
                box = ObjectInfoWithROI()
                ...
                box.xmax = xmax
                box.ymax = ymax
                # box_array.boxes.append(box) #changed by Y.S
                box_array.object_array.append(box)
                print '%s:%.2f xmin:%d ymin:%d xmax:%d ymax:%d' % (self.class_names[class_num], top_conf[i], xmin, ymin, xmax, ymax)
        
        box_array.header.frame_id = self.target_frame;
